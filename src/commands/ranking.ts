...
import { findMediaByRanking } from '@anilist/media';
...
import { sendMediaEmbed } from './find';
...
export class RankingCommand implements CommandHandler<SlashCommandContext> {
  ...
  async handle(context: SlashCommandContext): Promise<void> {
    ...
    const format = context.getString(`format`)
      ? MediaFormat[context.getString(`format`) as keyof typeof MediaFormat]
      : MediaFormat.TV;
    const season = context.getString(`season`)
      ? MediaSeason[context.getString(`season`) as keyof typeof MediaSeason]
      : undefined;
